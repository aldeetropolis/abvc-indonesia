...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      menuItem("Human Cases & Deaths", tabName = "casedeath", icon = icon("th")),
...
server <- function(input, output) {
  data <- eventReactive(input$run, {
    ...
    data <- enframe(pluck(res, "data")) |> unnest_wider(value) |> unnest(diseases) |> unnest(locations) |> 
      select(sourceUrl, sourceName, publishedTimestamp, diseases, locations, articleHeadline, articleSummary) |> as_tibble()
    return(data)
    })

  output$table <- render_gt(data())
}
