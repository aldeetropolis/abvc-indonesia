...
disease <- "15,141,83,164,84,166,55,53,88,38,63,13,4,205,10,43,26,153,156,162,6,110,113,136,212,215,216,224,264,200,41,40,39,37,64,239,47"
  data1 <- eventReactive(input$run_1, {
    ...
    data1 <- enframe(pluck(res1, "data")) |> unnest_wider(value) |> rowwise() %>% 
      mutate(across(where(is.list),
                    ~paste0(.,collapse = ", "))) |> 
      select(sourceUrl, sourceName, publishedTimestamp, articleHeadline, articleHeadlineTranslated, diseases, locations, affectedCategories, articleSummary, tags)
    return(data1)
  })
  ...
  data3 <- eventReactive(input$run_3, {
    ...
    data3 <- enframe(pluck(res3, "data")) |> unnest_wider(value) |> rowwise() %>% 
      mutate(across(where(is.list),
                    ~paste0(.,collapse = ", "))) |> 
      select(eventTitle, date, description, followUp, diseases, species, location, concernLevel, concernDescription, sourceConfidenceLevel, sourceConfidenceDescription, sources)
    return(data3)
  })
