...
# Dengue
url <- "https://developer.bluedot.global/diseases/?diseaseIds=55&includeDiseaseOverview=true&api-version=v1"
res <- GET(url, add_headers("Ocp-Apim-Subscription-Key" = "5f645982e25d4a729d7292b890a8ed31", "Cache-Control" = "no-cache")) |> content("text") |> 
disease <- enframe(pluck(res, "data")) |> unnest_wider(value)
acquisition <- enframe(pluck(res, "data", 1, "acquisitionModeGroups")) |> unnest_wider(value)
disease_info <- enframe(pluck(res, "data", 1, "diseaseOverview")) |> unnest_wider(value)
...
write.table(disease, "dengue.txt", row.names = FALSE)
