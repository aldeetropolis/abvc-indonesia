...
```{r weekly data}
weeklyData1 <- data |> filter(Date_reported >= "2022-12-25") |> 
  mutate(week = epiweek(Date_reported)) |> group_by(Country, week) |> 
  summarise(weekly_cases = sum(New_cases)) |> filter(weekly_cases >= 1)
weeklyData2 <- weeklyData1 |> pivot_wider(id_cols = week, names_from = Country, values_from = weekly_cases) |> 
  arrange(week) |> 
  fill("Brunei DS":"Vietnam", .direction = "downup")
weeklyData <- weeklyData2 |> pivot_longer(cols = c("Brunei DS":"Vietnam"), names_to = "Country", values_to = "weekly_cases") |> arrange(Country) |> 
  mutate(date = ymd("2022-12-25") + weeks(week)) |> 
  filter(week != 52, weekly_cases >= 0) |> arrange(Country)
...
```{r export}
png("weekly curve COVID-19 ASEAN 19 December 2023 area.png", units="px", width=3000, height=2250, res=300)
plot
dev.off()
```
...
# 7MA Graph
```{r}
maData <- data |> filter(Date_reported >= "2023-01-01") |> 
  group_by(Country) |> 
  mutate(ma_new_cases = round(rollmean(New_cases, k = 7, fill = 0), 2)) |> 
  ungroup()
maDeath <- data |> filter(Date_reported >= "2023-01-01") |> group_by(Date_reported) |> 
  summarise(new_deaths = sum(New_deaths)) |> 
  mutate(ma_new_deaths = round(rollmean(new_deaths, k = 7, fill = 0), 2))
```

```{r}
ggplot(data = maData, aes(date, ma_new_cases)) + geom_area(aes(fill = Country)) + 
  geom_line()
```
