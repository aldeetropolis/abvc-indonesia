...
```{r transform weekly from who data}
weeklyData1 <- who_data |> filter(Date_reported >= "2022-12-25") |> 
...
weeklyData <- weeklyData2 |> pivot_longer(cols = c("Brunei DS":"Vietnam"), names_to = "Country", values_to = "weekly_cases") |> arrange(Country) |> 
  ...
  filter(weekly_cases >= 0) |> arrange(Country)
weeklyDeath <- who_data |> filter(Date_reported >= "2022-12-25") |> 
