...
```{r plot}
plot <- ggplot(data) + 
  geom_line(aes(Date_reported, New_cases, color = Country), linewidth = 1.2) +
...
```{r export}
jpeg("epicurve COVID-19 ASEAN 6 July 2023.jpeg", units="px", width=4000, height=2250, res=300)
plot
dev.off()
```
...
plot <- ggplot(weeklyData) +
  geom_area(aes(date, weekly_cases, colour = Country, fill = Country)) +
  geom_line(data = weeklyDeath, aes(date, death*200, group = "Number of Death"),
  ...
  scale_x_date(breaks = "1 month", labels = date_format("%b-%Y"),
               limits = as.Date(c('2023-01-01','2023-07-06')), name = element_blank()) +
  theme(legend.title = element_blank()) +
  theme_hc()
plot
```
...
```{r export}
jpeg("weekly curve COVID-19 ASEAN 6 July 2023 area.jpeg", units="px", width=4000, height=2250, res=300)
plot
dev.off()
```
