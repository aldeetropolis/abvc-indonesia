...
```{r}
png("mpox world 09 November 2023.jpeg", units="px", width=4000, height=2250, res=300)
mpox_world_plot
dev.off()
```
...
## WHO Data Import
```{r}
who_data <- read_csv("mpox cases by country as of 30 September 2023.csv") |> filter(date >= "2023-01-01")
```

## Mpox cases per region
```{r}
region_data <- who_data |> 
  mutate(region = if_else(iso3 %in% c("BRN", "KHM", "IDN", "LAO", "PHL", "THA", "VNM", "MMR", "SGP", "MYS"), "ASEAN", if_else(who_region %in% c("South-East Asia Region", "Western Pacific Region"), "Asia Pacific", who_region))) |> 
  group_by(region) |> 
...
## Mpox cases in ASEAN
```{r}
asean_data <- read_csv("mpox cases by country as of 30 September 2023.csv") |> 
...
## ASEAN
```{r}
ebsUrl <- "https://developer.bluedot.global/casecounts/diseases/64?locationIds=1880251,%201605651,%201694008,%201820814,%201733045,%201643084,%201831722,%201327865,%201655842,1562822&startDate=2023-01-01&endDate=2023-11-03&api-version=v1"
...
ebsData |> group_by(locationName) |> summarise(n = sum(totalReportedCases))
```

```{r}
ebsUrl <- paste0("https://developer.bluedot.global/casecounts/locations/1562822?startDate=", startDate, "&endDate=", endDate, "&isAggregated=false&includeSources=true&api-version=v1")
